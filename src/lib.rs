...
mod tests {
	...
	fn test_component_name() {
		...
		assert_eq!(
			parse_component_name("RTR"), 
			Ok(("", Component::ROUTER))
		);
		assert_eq!(
			parse_component_name("SSH"), 
			Ok(("", Component::SSH))
		);
		assert_eq!(
			parse_component_name("STG"), 
			Ok(("", Component::STAGING))
		);
	}
	...
	fn test_parse_cf_app_log_no_message() {
		let entry = parse_cf_app_log(
			r#"2021-09-28T17:00:09.36+0900 [RTR/0] OUT"#
		);
        assert!(entry.is_ok(), "res: {:#?}", entry);
